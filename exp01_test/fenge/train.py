...
# 训练日志
writer = SummaryWriter(os.path.join(root_path, 'log/bladder/train', model_name + str(int(time.time()))))
val_writer = SummaryWriter(os.path.join(root_path, 'log/bladder/val', model_name+ str(int(time.time()))))

# 训练集路径
train_path = r'/HOME/scz0490/run/yy/p/ddsm-data/qiepian'
val_path = r'/HOME/scz0490/run/yy/p/ddsm-data/qiepian'
...
def main():
    ...
    # 训练集加载
    train_set = Dataset(train_path, 'train',crop=center_crop,transform=input_transform)
    ...
    if scheduler_type == 'StepLR':
        ...
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=gamma, patience=4,threshold=1e-2)
    else:
        scheduler = None
    ...
    train(train_loader, val_loader, net, criterion, optimizer, scheduler, None, early_stopping, start_epoch,end_epoch, iters)


def train(train_loader, val_loader, net, criterion, optimizer, scheduler, warm_scheduler, early_stopping, start_epoch,num_epoches, iters):
    epoch=0
    for epoch in range(start_epoch, num_epoches + 1):
        ...
        for batch, ((inputs, mask), file_name) in enumerate(train_loader, 1):
            ...
            recall,PA_b,acc,precision,dice,pixel_labeled = pixel_accuracy(output, mask)
            if pixel_labeled == 0:
                recall = 0.0
                precision = 0.0
                dice = 0.0
            ...
            string_print = 'epoch: {}, iters: {}, loss: {:.4}, dice:{:.4},recall:{:.4},precision:{:.4},time: {:.2}'\
                .format(epoch, iters, loss.data.cpu(), dice, recall, precision, time.time() - st)
        ...
        print('epoch {}/{}, train_loss: {:.4}, dice(f1):{:.4},acc:{:.4},recall:{:.4},precision:{:.4}'
              .format(epoch, num_epoches, train_loss,train_class_dice, train_acc, train_recall,train_precision))
        ...
        for val_batch, ((inputs, mask), file_name) in tqdm(enumerate(val_loader, 1)):
            ...
            valrecall,valPA_b,valacc,valprecision,valdice,valpixel_labeled = pixel_accuracy(pred, mask)
        ...
        print('val_loss:{:.4}, dice(f1):{:.4},recall:{:.4},precision:{:.4},acc:{:.4}'
              .format(val_loss, val_class_dice, val_recall,val_precision,val_acc))
